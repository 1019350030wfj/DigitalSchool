...
public class ReceiveAction1 {
    ...
    private static ReceiveAction1 sInstance;

    private ReceiveAction1() {
        mMsgCallback = new SparseArray<>();
    }

    public static ReceiveAction1 getInstance() {
        if (sInstance == null) {
            synchronized (SendAction.class) {
                if (sInstance == null) {
                    sInstance = new ReceiveAction1();
                }
            }
        }
        return sInstance;
