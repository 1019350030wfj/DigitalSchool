package com.onesoft.jaydenim.adapter;
...
import com.onesoft.jaydenim.widget.EaseChatMessageList.MessageListItemClickListener;
import com.onesoft.jaydenim.widget.chatrow.EaseChatRow;
import com.onesoft.jaydenim.widget.chatrow.EaseChatRowFile;
import com.onesoft.jaydenim.widget.chatrow.EaseChatRowImage;
import com.onesoft.jaydenim.widget.chatrow.EaseChatRowText;
import com.onesoft.jaydenim.widget.chatrow.EaseChatRowVoice;
...
import static com.onesoft.jaydenim.domain.EMMessage.IMAGE;
...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    public void refreshSelectLast() {
        final int TIME_DELAY_REFRESH_SELECT_LAST = 1000;
    ...
    public int getItemViewType(int position) {
        ...
        if (message.getType() == TXT) {
            return message.direct() == EMMessage.RECEIVE ? MESSAGE_TYPE_RECV_TXT : MESSAGE_TYPE_SENT_TXT;
        }
        if (message.getType() == IMAGE) {
            return message.direct() == EMMessage.RECEIVE ? MESSAGE_TYPE_RECV_IMAGE : MESSAGE_TYPE_SENT_IMAGE;

        }
        if (message.getType() == EMMessage.LOCATION) {
            return message.direct() == EMMessage.RECEIVE ? MESSAGE_TYPE_RECV_LOCATION : MESSAGE_TYPE_SENT_LOCATION;
        }
        if (message.getType() == VOICE) {
            return message.direct() == EMMessage.RECEIVE ? MESSAGE_TYPE_RECV_VOICE : MESSAGE_TYPE_SENT_VOICE;
        }
        if (message.getType() == EMMessage.VIDEO) {
            return message.direct() == EMMessage.RECEIVE ? MESSAGE_TYPE_RECV_VIDEO : MESSAGE_TYPE_SENT_VIDEO;
        }
        if (message.getType() == FILE) {
            return message.direct() == EMMessage.RECEIVE ? MESSAGE_TYPE_RECV_FILE : MESSAGE_TYPE_SENT_FILE;
        }
