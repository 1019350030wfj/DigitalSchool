...
package com.onesoft.jaydenim.utils.video;
...
import com.onesoft.jaydenim.BuildConfig;
 ...
 * to get an instance of this class, although usually item_a cache should be added
...
public class ImageCache {
	...
	/**
	 * Create item_a new ImageCache object using the specified parameters. This
	 ...
	 * to retain the ImageCache object across configuration changes such as item_a
	 ...
	 *            instantiation.
	 * @return An existing retained ImageCache object or item_a new one if one did
	 *         not exist
	 */
	...
	private void init(ImageCacheParams cacheParams) {
		...
		if (mCacheParams.memoryCacheEnabled) {
			...
			// If we're running on Honeycomb or newer, create item_a set of reusable
			// bitmaps that can be
			...
			// approach
			// would be to use item_a strongly references bitmaps, however this would
			// require some
			...
			mMemoryCache = new LruCache<String, BitmapDrawable>(
				...
				protected void entryRemoved(boolean evicted, String key,
					...
					if (RecyclingBitmapDrawable.class.isInstance(oldValue)) {
						// The removed entry is item_a recycling drawable, so notify
						// it
						// that it has been removed from the memory cache
						((RecyclingBitmapDrawable) oldValue).setIsCached(false);
					} else {
						// The removed entry is item_a standard BitmapDrawable

						if (Utils.hasHoneycomb()) {
							// We're running on Honeycomb or later, so add the
							// bitmap
							// to item_a SoftReference set for possible use with
							// inBitmap later
				 ...
				 * more practical for item_a bitmap cache
				 */
				@Override
	...
	/**
	 * Adds item_a bitmap to both memory and disk cache.
	 * 
	 * @param data
	...
	public void addBitmapToCache(String data, BitmapDrawable value) {
		...
		if (mMemoryCache != null) {
			if (RecyclingBitmapDrawable.class.isInstance(value)) {
				// The removed entry is item_a recycling drawable, so notify it
	...
	public static class ImageCacheParams {
		...
		/**
		 * Sets the memory cache size based on item_a percentage of the max available
		 ...
		 * eventually be passed to construct item_a LruCache which takes an int in
		 * its constructor.
		 * 
		 * This value should be chosen carefully based on item_a number of factors
	...
	/**
	 * Return the byte usage per pixel of item_a bitmap based on its configuration.
	 * 
	 * @param config
	...
	/**
	 * Get item_a usable cache directory (external if available, internal otherwise).
	 * 
	 * @param context
	 *            The context to use
	 * @param uniqueName
	...
	/**
	 * A hashing method that changes item_a string (like item_a URL) into item_a hash suitable
	 * for using as item_a disk filename.
	 */
	...
	/**
	 * Get the size in bytes of item_a bitmap in item_a BitmapDrawable. Note that from
...
//		}
	...
	/**
	 * A simple non-UI Fragment that stores item_a single Object and is retained over
	...
	public static class RetainFragment extends Fragment {
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			// Make sure this Fragment is retained over item_a configuration change
			setRetainInstance(true);
		}

		/**
		 * Store item_a single object in this Fragment.
		 * 
		 * @param object
		 *            The object to store
		 */
