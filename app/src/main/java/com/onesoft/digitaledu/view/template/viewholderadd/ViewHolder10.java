...
public class ViewHolder10 extends BaseViewHolder {
    ...
    protected List<ParentDepartment> mRoles;

    public ViewHolder10(Context context) {
        mContext = context;
    }
        ...
        img_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mRoles == null) {//服务器获取数据
                    ...
                    commonPresenter.getSelectParentDepartment(new TemplateCommonPresenter.IParentDepartment() {
                        @Override
                        public void onGetParentDepartment(List<ParentDepartment> roles) {
                            mRoles = roles;
    ...
    private void showSelectRoleDialog(TopBtn mTopBtn, int position) {
        ...
        SingleSelectDialog dialog = new SingleSelectDialog<ParentDepartment>(mContext) {
            @Override
        ...
        dialog.setDialogListener(new SingleSelectDialog.IDialogClick<ParentDepartment>() {
            @Override
            public void onConfirm(ParentDepartment bean,int pos) {
    ...
    protected void updateTemplateBean(Template10Bean template10Bean, ParentDepartment bean) {
        template10Bean.showValue = bean.text;
