 ...
 */
public class ReplyDocActivity extends ToolBarActivity<SendDocPresenter> implements IAddView {
    ...
    public static final int TYPE_REPLAY = 0X116;
    public static final int TYPE_FORWARD = 0X117;
    ...
    private BoxDetail mBoxBean;
    private int mAttachPos;
    ...
    private void getDataFromForward() {
        ...
        if (bundle != null) {
            mFromType = bundle.getInt("mFromType");
            mBoxBean = (BoxDetail) bundle.getSerializable("boxBean");
            if (TYPE_FORWARD == mFromType) {
                mAttachPos = bundle.getInt("pos");
            }
        }
    }

    public static void startReply(Context context, BoxDetail boxBean, int fromType) {
        Intent intent = new Intent(context, ReplyDocActivity.class);
        intent.putExtra("boxBean", boxBean);
        intent.putExtra("mFromType", fromType);
        context.startActivity(intent);
    }

    public static void startForward(Context context, BoxDetail boxBean, int pos, int fromType) {
        Intent intent = new Intent(context, ReplyDocActivity.class);
        intent.putExtra("boxBean", boxBean);
    ...
    public void initData() {
        ...
        mGridAdapter.setAddClickListener(new FilePickGridAdapter.OnAddClickListener() {
            ...
            public void onOpenFile(String path) {//打开文件
                try {
                    FileUtils.openFile(new File(path), ReplyDocActivity.this);
                } catch (Exception e) {
                    Toast.makeText(ReplyDocActivity.this, "无法打开此文件", Toast.LENGTH_SHORT).show();
                    e.printStackTrace();
                }
            }
            ...
            public void onOpenImage(ArrayList<String> paths, int pos) {//查看大图
                LookBigPicActivity.showBigPic(ReplyDocActivity.this, paths, pos);
            }
        });
        ...
        if (mFromType == TYPE_REPLAY) {//回复
            mTxtRecipient.setText(mBoxBean.name);
            mEditTitle.setText("Re:" + mBoxBean.title);
            mImageRecipient.setVisibility(View.INVISIBLE);
            touserids = mBoxBean.fromuser;//发件人，也就是回复的收件人
            ArrayList<FilePickBean> filePickBeens = new ArrayList<>();
            for (Attach attach : mBoxBean.attach) {
                FilePickBean filePickBean = new FilePickBean("1".equals(attach.is_pic) ? FilePickBean.TYPE_IMAGE : FilePickBean.TYPE_FILE,
                        PathUtil.getInstance().getDownloadDir() + attach.new_name);
                filePickBeens.add(filePickBean);
            }
            mGridAdapter.addList(filePickBeens);
        } else if (mFromType == TYPE_FORWARD) {//转发
            mEditTitle.setText("Forward:" + mBoxBean.title);
            Attach attach = mBoxBean.attach.get(mAttachPos);
            if (attach != null) {
                mGridAdapter.addItem(new FilePickBean("1".equals(attach.is_pic) ? FilePickBean.TYPE_IMAGE : FilePickBean.TYPE_FILE,
                        PathUtil.getInstance().getDownloadDir() + attach.new_name));
            }
        }
    ...
    private void initPermissions() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ...
            GalleryPick.getInstance().setGalleryConfig(galleryConfig).open(ReplyDocActivity.this);
        }
    }
            ...
            public void onClickShot() {
                if (FilePickGridAdapter.MAX_COUNT - mGridAdapter.getDataCount() - 1 > 0) {
                    ...
                    if (intent.resolveActivity(getPackageManager()) != null) {
                        mTmpFile = FileUtils.createTmpFile(ReplyDocActivity.this);
                        Uri photoUri = Uri.fromFile(mTmpFile); // 传递路径
                        intent.putExtra(MediaStore.EXTRA_OUTPUT, photoUri);// 更改系统默认存储路径
                        startActivityForResult(intent, REQUEST_CAMERA);
                    } else {
                        Toast.makeText(ReplyDocActivity.this, "No Camera", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    RemindUtils.showDialog(ReplyDocActivity.this, getResources().getString(R.string.max_count));
                }
            }
            ...
            public void onClickFile() {
                if (FilePickGridAdapter.MAX_COUNT - mGridAdapter.getDataCount() - 1 > 0) {
                    new MaterialFilePicker()
                            .withActivity(ReplyDocActivity.this)
    ...
    public void initListener() {
        mImageTypeReceipt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {//是否回执
                SingleSelectDialog dialog = new SingleSelectDialog<SingleSelectBean>(ReplyDocActivity.this) {
                    @Override
                    public SingleSelectAdapter getAdapter() {
                        return new SingleSelectAdapter(ReplyDocActivity.this) {
                            @Override
        ...
        mImageRecipient.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {//收件人
                TreeActivity.startRecipientActivity(ReplyDocActivity.this, REQUEST_CODE_TEACHER);
            }
        });
