...
public class ThirdMainToRecoveryPresenter extends BasePresenter<IThirdToMainView> {

    private TemplateHandleDataHelper mTemplateHandleDataHelper;

    public ThirdMainToRecoveryPresenter(Context context, IThirdToMainView iView) {
        super(context, iView);
        mTemplateHandleDataHelper = new TemplateHandleDataHelper();
    }

    public void getData(final ThirdToMainBean mainBean, TopBtn topBtn, int page) {
        String url = HttpUrl.THREE_MENU_TO_DETAIL + topBtn.menuID + "&page=" + page
                + "&template_id=" + topBtn.template_id
                + "&user_id=" + SPHelper.getUserId(mContext) + "&cid=" + mainBean.id;
        HttpHandler.getInstance(mContext).getAsync(mContext, url, new HttpHandler.ResponseCallback() {
            ...
            public void onResponse(int code, String response) {
                handleResponse(mainBean, response);

            }
        });
    }

    public void handleResponse(ThirdToMainBean mainBean, String response) {
        //1、取top_btn 数组
        //2、sort 字符串， 排序菜单栏
        ...
        try {
            ...
            for (int i = 0, length = top_btnArray.length(); i < length; i++) {
                ...
                /*============添加页要显示的字段=============*/
                mTemplateHandleDataHelper.convertServerDataToClientData(mainBean, topBtn, jsonFieldName, null);
                /*============添加页要显示的字段=============*/
                ...
                for (int j = 0, size = listField.length; j < size; j++) {
                    ...
                    if (tempObject instanceof String) {//如果是字符串
                        ...
                        for (int t = 0, tsize = itemJsonArray.length(); t < tsize; t++) {//循环遍历出每个操作按钮
                            ...
                            topBtn.notice = jsontop.getString("notice");
                            /*============添加页要显示的字段=============*/
                            mTemplateHandleDataHelper.convertServerDataToClientData(mainBean, topBtn, jsonFieldName, object);
                            /*============添加页要显示的字段=============*/
                            topBtn.parent_id = jsontop.getString("parent_id");
                            topBtn.children_id = jsontop.getString("children_id");
     ...
     * @param page
     */
    public void getDataBySort(final ThirdToMainBean keyValueBean, TopBtn topBtn,MenuTitle menuTitle, String id, int page) {
        String url = HttpUrl.THREE_MENU_TO_DETAIL + id +
                "&page=" + page + "&sort_key=" + menuTitle.id + "&template_id=" + topBtn.template_id
                + "&user_id=" + SPHelper.getUserId(mContext)
                + "&cid=" + keyValueBean.id + "&sort_val=" + (menuTitle.isUp ? "asc" : "desc");
        HttpHandler.getInstance(mContext).getAsync(mContext, url, new HttpHandler.ResponseCallback() {
            @Override //回调是异步的，如果有UI操作，记得切换到主线程
            public void onError(Request request, Exception e) {
            ...
            @Override
            public void onResponse(int code, String response) {
                handleResponse(keyValueBean, response);
            }
        });
    }
